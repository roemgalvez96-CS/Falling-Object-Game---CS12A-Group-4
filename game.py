...
from constants import (
    ...
    FRENZY_THRESHOLD, FRENZY_DURATION_SEC, FRENZY_SPAWN_MULT,
    MAX_LIVES,
    BG_IMG, HEART_IMG, FRENZY_BANNER_IMG, GOLDEN_APPLE_IMG, ROTTEN_APPLE_IMG,
    SHIELD_ICON_IMG, MAGNET_ICON_IMG, GOLDEN_APPLE_ICON,
    APPLE_SCORE_IMG, COIN_SCORE_IMG, COIN_IMG,
    TROPHY_HIGHSCORE_IMG, SCORE_PANEL_IMG,
)
from assets import PC, PlayBG, G_O, LVL, OW, FRENZY_BGM
...
class Game:
    def __init__(self):
        ...
        # ── Game Over panels ─────────────────────────────────────────────────
        TROPHY_HIGHSCORE_SIZE = (320, 314)   # ← change size here
        SCORE_PANEL_SIZE      = (340, 195)   # ← change size here
        ...
        # ── Power-up icons ───────────────────────────────────────────────────
        ICON_SIZE = (52, 52)   # ← change to resize power-up icons
        ...
        # HUD score panels
        APPLE_SCORE_SIZE = (160, 52)   # ← apple score panel size
        COIN_SCORE_SIZE  = (160, 52)   # ← coin/best score panel size
        self.apple_score_img = pygame.transform.scale(
            pygame.image.load(APPLE_SCORE_IMG).convert_alpha(), APPLE_SCORE_SIZE
        )
        self.coin_score_img = pygame.transform.scale(
            pygame.image.load(COIN_SCORE_IMG).convert_alpha(), COIN_SCORE_SIZE
        )
    ...
    def reset_game(self):
        ...
        self.pending_spawns     = []  # list of (obj_type, delay_frames)
        ...
        self.rotten_magnet_duration = 5 * FPS  # 5 seconds

        # ── Power-up state ────────────────────────────────────────────────────
        COOLDOWN = 120 * FPS   # 2 minutes in frames
        DURATION = 10  * FPS   # 10 seconds in frames
        ...
        # Apple Frenzy state
        self.worm_chance_increase = 0.03  # starts at +3% per level up
    ...
    def spawn_object(self):
        ...
        coin_on_screen = any(o.type == 'coin' for o in self.falling_objects)
        if not coin_on_screen and random.random() < 0.15:
            self._spawn_one('coin')
            return

        golden_on_screen = any(o.type == 'golden_apple' for o in self.falling_objects)
        if not golden_on_screen and random.random() < 0.02:
            self._spawn_one('golden_apple')
            return

        # ← change these rates individually (must not exceed 1.0 combined)
        WORM_CHANCE         = 0.30 + (self.worm_chance_increase * self.difficulty_level)  # grows each level
        WORM_CHANCE         = min(WORM_CHANCE, 0.65)  # cap at 65%
        ROTTEN_APPLE_CHANCE = 0.15   # ← rotten apple spawn rate
        # Apple always gets the remainder — guaranteed to always spawn something

        roll = random.random()
        if roll < WORM_CHANCE:
            self._spawn_one('bomb')
            # Queue 2nd worm with a random delay (20–60 frames)
            ...
            # Queue 2nd rotten apple with a random delay
            self.pending_spawns.append(('rotten_apple', random.randint(20, 60)))
        else:
            self._spawn_one('good')
            # Queue 2nd apple with a random delay
    ...
    def update(self):
        ...
        if self.magnet_active:
            ...
            for obj in self.falling_objects[:]:
                if obj.type in ('good', 'coin', 'golden_apple', 'heart'):
        ...
        # ── Rotten magnet: pull worms toward player ───────────────────────────
        if self.rotten_magnet_active:
            target_x = self.player.rect.centerx
            target_y = self.player.rect.centery
            for obj in self.falling_objects[:]:
        ...
        for obj in self.falling_objects[:]:
            ...
            if obj.check_collision(self.player):
                ...
                if obj.type == 'good' and not self.frenzy_active and not self.shield_active:
                    self.lives -= 1
                    OW.play()
                    OW.set_volume(1.0)
                    if self.lives <= 0:
        ...
        if not self.frenzy_active:
            if self.score >= self.last_difficulty_score + SPEED_THRESHOLD and self.score > 0:
                ...
                # Increase worm chance each level: +3% at level 2, +4% at level 3, +5% ...
                self.worm_chance_increase += 0.01  # grows by 1% each level
                self.worm_chance_increase  = min(self.worm_chance_increase, 0.10)  # cap at +10%
    ...
    def _draw_hud(self):
        ...
        apple_img = pygame.transform.scale(self.apple_score_img, (BAR_W, BAR_H))
        coin_img  = pygame.transform.scale(self.coin_score_img,  (BAR_W, BAR_H))

        # ── LEFT: score bars ──────────────────────────────────────────────────
        apple_x, apple_y = HUD_X, HUD_Y
        coin_x,  coin_y  = HUD_X, HUD_Y + BAR_H + BAR_GAP

        self.screen.blit(apple_img, (apple_x, apple_y))
        self.screen.blit(coin_img,  (coin_x,  coin_y))

        score_txt = self.small_font.render(str(self.score), True, WHITE)
        self.screen.blit(score_txt, (
            apple_x + BAR_W - score_txt.get_width() - 12,
            apple_y + (BAR_H - score_txt.get_height()) // 2
        ))
        coin_txt = self.small_font.render(str(self.coins), True, GOLD)
        self.screen.blit(coin_txt, (
            coin_x + BAR_W - coin_txt.get_width() - 12,
            coin_y + (BAR_H - coin_txt.get_height()) // 2
        ))

        # ── LEFT: power-up icons ──────────────────────────────────────────────
        icon_y1 = coin_y + BAR_H + ICON_GAP
        ...
        self.screen.blit(level_txt, (
            WINDOW_WIDTH // 2 - level_txt.get_width() // 2, HUD_Y + 4
        ))

        if not self.frenzy_active and self.difficulty_level > 0 \
                and self.score - self.last_difficulty_score < 5:
            wave_txt = self.small_font.render(f"WAVE {self.difficulty_level + 1}!", True, GOLD)
        ...
        # ── RIGHT: golden apple counter ───────────────────────────────────────
        ga_icon = self.golden_apple_icon_img
        ga_x    = WINDOW_WIDTH - 8 - ga_icon.get_width()
        ga_y    = hy + HEART_SIZE + 6
        self.screen.blit(ga_icon, (ga_x, ga_y))
        ga_txt = self.small_font.render(f"x{self.golden_apples_caught}", True, GOLD)
        self.screen.blit(ga_txt, (
            ga_x - ga_txt.get_width() - 4,
            ga_y + ga_icon.get_height() // 2 - ga_txt.get_height() // 2
        ))
    ...
    def _draw_game_over(self):
        ...
        SCORE_PANEL_Y        = 260    # ← score panel vertical position
        SCORE_BOX_TOP_RATIO  = 0.20   # ← where yellow box starts (score panel)
        TROPHY_PANEL_Y       = 140    # ← trophy panel vertical position
        TROPHY_BOX_TOP_RATIO = 0.60   # ← where yellow box starts (trophy panel)
            ...
            for event in pygame.event.get():
                ...
                if self.game_over:
                    ...
                    if self.btn_main_menu.handle_event(event):
                        PlayBG.stop(); G_O.stop(); FRENZY_BGM.stop()
